 ...
 * @param tower 
 */
void EntityHandler::UpdateEntities(sf::RenderWindow& window, float deltaTime, Tower& tower)
{
    for (auto& entity : mEnemies)
    {
        if (!entity) continue;
        entity->update(window, deltaTime);

        Enemies* enemy = dynamic_cast<Enemies*>(entity.get());
        if (enemy && !enemy->isDead() && enemy->isAttacking())
            enemy->attackTower(tower, deltaTime);
    }
